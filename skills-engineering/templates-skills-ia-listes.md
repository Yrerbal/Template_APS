...
**Entrées**
- [param_1] (texte, obligatoire) : [description courte]
- [param_2] (liste, optionnel, défaut : [valeur]) : [description courte]
...
- "répond à tout" (skill trop large) : imprévisible, non testable → découper en skills atomiques
- contraintes absentes : l'agent déborde du périmètre → toujours définir ce qu'il ne fait PAS
- exemple absent : le comportement est ambigu → toujours inclure au moins un exemple
- déclencheur vague ("si besoin") : activation incohérente → condition précise et exclusive
- sortie non spécifiée : format variable d'un appel à l'autre → format, longueur, ton explicites
